...
// CODE HERE
const changeColor = sayHelloButton => {
    sayHelloButton.target.style.color = 'white'
    sayHelloButton.target.style.backgroundColor = 'black'
...
// CODE HERE
const changeColorBack = sayHelloButton => {
    sayHelloButton.target.style.color = 'black'
    sayHelloButton.target.style.backgroundColor = '#EFEFEF'
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/SirLancealot')
    .then(response =>{
        console.log(response.data)
        document.getElementById('repeat-text').setAttribute('textContent', response.data)
        document.querySelector('#repeat-text').style.display = 'block'
    })
    .catch(err => console.log(err))
    
}
let repeatBtn = document.querySelector('#repeat-button').addEventListener('click', repeatMyParam)
...
// CODE HERE
const queryFunc = () =>{
    axios.get('http://localhost:3000/query-test/?name=brandon')
    .then(response =>{
        console.log(response.data)
    })
    .catch(err => console.log(err))
